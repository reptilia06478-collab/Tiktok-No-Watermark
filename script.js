...
(function initIntroTransition() {
  ...
  cont?.addEventListener('click', closeIntro);
  if (reduceMotion) closeIntro();
  else setTimeout(closeIntro, 3900);

  document.addEventListener('click', (e) => {
    const link = e.target.closest('a[href]');
    if (!link) return;
    const href = link.getAttribute('href');
    if (!href || href === '#' || link.target === '_blank' || href.startsWith('javascript:')) return;
    if (link.dataset.noTransition === 'true') return;
    const isExternal = /^https?:\/\//i.test(href);
    const sameOrigin = !isExternal || href.startsWith(location.origin);
  ...
  // External TikTok link: show the same short transition before leaving.
  document.querySelectorAll('a[href*="vt.tiktok.com/ZSqgwpFkM"]').forEach((link) => {
